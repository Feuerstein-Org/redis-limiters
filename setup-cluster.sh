...
for host in 10.15.0.2 10.15.0.3 10.15.0.4 10.15.0.5 10.15.0.6 10.15.0.7; do
...
redis-cli --cluster create 10.15.0.2:6379 10.15.0.3:6379 10.15.0.4:6379 --cluster-yes
...
# Add slaves
redis-cli --cluster add-node 10.15.0.5:6379 10.15.0.2:6379 --cluster-slave --cluster-master-id $master1
redis-cli --cluster add-node 10.15.0.6:6379 10.15.0.2:6379 --cluster-slave --cluster-master-id $master2
redis-cli --cluster add-node 10.15.0.7:6379 10.15.0.2:6379 --cluster-slave --cluster-master-id $master3

# Force rebalance to ensure all slots are covered (no data, so safe)
redis-cli --cluster rebalance --cluster-use-empty-masters 10.15.0.2:6379 --cluster-yes
...
while true; do
    ...
    redis-cli --cluster check 10.15.0.2:6379
    if redis-cli --cluster check 10.15.0.2:6379 | grep -q "[OK]"; then
